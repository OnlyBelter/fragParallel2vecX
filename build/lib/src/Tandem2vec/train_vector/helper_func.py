...
from ..._base import SELECTED_MD, PRMIER_NUM
...
def train_fasttext_model(infile_name, outfile_name=None, dim=100, ws=4, min_count=3, n_jobs=1,
    ...
    # sg=0 means using 'cbow' model, size means dim, window means ws, iter means epoch,
    ...
    model = fasttext.FastText(corpus, sg=sg, size=dim, window=ws, iter=epoch,
