...
from ._fragment_assistant import get_clique_smiles
...
class Mol2Network:
    def __init__(self, smiles, frag_id2mol_inx, edges):
        """
        represent a molecule by graph
        :param smiles: the SMILES of this molecule, eg: "CC(CC(=O)NC1=CC=C(C=C1)CCC(=O)O)C2=CC=CC=C2" (CID: 28376780)
        :param frag_id2mol_inx: fragment id to atom index in this molecule, eg:
        ...
        :param edges: the relation between two fragment, eg:
            [["0", "14"], ["1", "2"], ["1", "14"], ["2", "15"], ["3", "15"], ["4", "5"], ["4", "15"],
            ["5", "12"], ["6", "7"], ["6", "12"], ["7", "8"], ["8", "9"], ["11", "13"], ["11", "14"]]
        """
        # self.smiles = smiles
        self.edges = [(int(i), int(j)) for i, j in edges]
        self.mol = get_mol(smiles)
        self.id2mol_inx = frag_id2mol_inx

        self.id2smiles = self._get_frag2smiles()
        self.g = self._get_graph()
        self.n2n = self._get_n2n()
        self.end_points = self._get_end_points()

    def _get_n2n(self):
        """
        n2n: the neighbors of each fragment (node id and it's neighbors), eg:
        -  {"0": [14], "1": [2, 14], "2": [1, 15], "3": [15],
            "4": [5, 15], "5": [4, 12], "6": [7, 12], "7": [6, 8],
            "8": [7, 9], "9": [8], "11": [13, 14], "12": [5, 6],
            "13": [11], "14": [0, 1, 11], "15": [2, 3, 4]}
        :return:
        """
        n2n = {}
        for x, y in self.edges:  # get neighbors for each fragment
            if x not in n2n:
                n2n[x] = []
            if y not in n2n:
                n2n[y] = []
    ...
    def _get_frag2smiles(self):
        """
        the SMILES string of each fragment, eg:
        -  {"0": "CC", "1": "CC", "2": "CC", "3": "C=O", "4": "CN",
            "5": "CN", "6": "CC", "7": "CC", "8": "CC",
            "9": "O=CO", "11": "CC", "12": "C1=CC=CC=C1",
            "13": "C1=CC=CC=C1", "14": "C", "15": "C"}
        :return:
        """
        # get SMILES for each fragment
        frag_id2smiles = {}
        for frag_id, atoms in self.id2mol_inx.items():
            frag_id2smiles[frag_id] = get_clique_smiles(self.mol, atoms)
        return frag_id2smiles
    ...
    def _get_end_points(self):
        # only have one neighbor node TODO: and this neighbor node is not a singleton (single atom)
        # end_points = [i for i in self.n2n if (len(self.n2n[i]) == 1) and
        #               (len(self.id2mol_inx[self.n2n[i][0]]) != 1)]
    ...
    def get_mol_path(self):
        ...
        all_paths = []
        all_paths_dic = {}  # using dict to remove duplicated paths
        ...
        if len(self.n2n) >= 2:
            for pairs in end_pairs:
                ...
                shortest_path = tuple(list(shortest_path)[0])
            ...
            for path in paths_with_attr:
                # print('>>>>>>>>>> this is path <<<<<<<<<')
                # print(path)
                all_paths_dic[path['path']] = 1
            all_paths = list(all_paths_dic.keys())
...
def get_fragment_sentence(frag2num_fp, cid2frag_fp, result_fp, result_fp2, replace_by_id=True):
    ...
    or replace SMILES of fragment by fragment id for saving storage space
    ...
    :param result_fp2: file path of fragment id or fragment SMILES sentence (separated by space)
    :param replace_by_id: ??
    :return:
    """
    ...
    f_handel_frag_sentence = open(result_fp2, 'a', encoding='utf-8')

    with open(cid2frag_fp, 'r') as f_handle:
        counter = 0
        for i in tqdm(f_handle):
            # if counter % 500000 == 0:
            ...
            if replace_by_id:  # replace fragment SMILES by id for saving storage
                ...
                frags_id = frags
                f_handel_frag_sentence.write(' '.join(frags_id) + '\n')
            counter += 1
    f_handel_frag_sentence.close()
